...
flowchart TD
    ...
    CI -- 실패 --> RE{로컬에서 재현되나}
    RE -- 된다 --> C
    RE -- 안 된다 --> HE[NEED_HUMAN · 환경]
    CI -- 통과 --> RV{⑦ 사람 판단}
    RV -- 요구사항대로 안 됐다 --> C
    RV -- 요구사항을 바꾼다 --> X([루프 밖 · 차감 없이 종료<br/>사람이 이슈를 닫는다<br/>재개는 새 Issue · 새 PR])
    RV -- 승인 --> M([⑧ 머지 = 루프 종료])
...
| 갈래 | 어디로 | 회차 |
|---|---|---|
| **승인** | ⑧ 머지 = **루프 종료** → 이슈 닫기 | `n/N 사용` 기록 |
| **요구사항대로 안 됐다** | ③ 구현으로 복귀 | **n+1** — 계약이 그대로면 AI 의 몫 (§3-2) |
| **요구사항을 바꾼다** | **루프 밖.** 사람이 이슈를 닫는다 | **차감 없음** · 재개는 새 Issue · 새 PR |

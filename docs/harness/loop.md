...
**한 줄:** **상한 N 을 받고 시작**해서, 고치고 `npm run verify` 를 돌리기까지가 1회.
n 이 N 에 닿으면 **중단**하고 계약을 의심한다.
...
1. **상한 없이 시작하지 않는다.** 루프는 `n / N` 으로 열린다. N 이 없으면 시작 자체를 안 한다.
2. **n 이 N 에 닿으면 중단한다.** 넘겨서 한 번 더 해보는 일은 없다.
3. **루프는 판정을 바꾸지 못한다.** 고칠 수 있는 것은 코드뿐이다.
4. **1회 = 고치고 `npm run verify` 를 돌리기까지.** 실행이 회차의 경계다.
...
```mermaid
flowchart TD
    A([루프 시작 · 상한 N 을 받는다<br/>계약 6번 · 기본 3]) --> C[구현 — 코드만 고친다]
...
**루프는 `n / N` 으로 열린다.** N 을 모르면 시작하지 않는다 — 셀 수 없는 루프는 멈출 줄도 모른다.

| 무엇 | 어디서 |
|---|---|
| 상한 N | Issue 계약 **6번** ([ssot.md §4](ssot.md)) |
| 기본값 | **3회** — 6번이 비어 있을 때만 |
| 현재 회차 n | 이슈의 마지막 루프 댓글 (§5). 없으면 0 |

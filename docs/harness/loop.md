...
| 기본값 | [ssot.md §7](ssot.md) 의 전역 기본값 — 6번이 비어 있을 때만 |
| 현재 회차 n | 그 이슈의 마지막 루프 댓글 (§5). 없으면 0 |
| **열지 않는 경우** | `contract-draft` 라벨이 붙어 있다 — 아직 계약이 아니라 초안이다 |

**`contract-draft` 는 AI 가 떼지 않는다.** 붙이는 것은 에이전트, 떼는 것은 사람이다 —
에이전트가 초안을 쓰고 스스로 확정하면 계약을 AI 가 정하는 것이 된다 ([ssot.md §4](ssot.md)).
사람이 라벨을 떼고 `계약 확정` 댓글을 달면 그때 상한을 확정하고 연다.
...
| **CI 실패** (§6) | **0회** — 같은 코드를 다른 환경에서 재판정한 것이라 새 회차가 아니다 |
| 갈래 선택 · ⑥-b 리뷰 실행 · 라벨 조작 | 0회 |
| **사람이 요구사항을 바꿈** | **차감 없음 · 루프 종료** — 아래 §3-2 |

**차감은 「실행」에 붙는다.** 회차를 늘리는 것은 `npm run verify` 를 돌리는 행위 하나뿐이고,
판단 · 기록 · 자문은 아무리 오래 걸려도 0회다. CI 실패가 0회인 것도 같은 이유다 —
고치면 어차피 로컬 verify 를 돌려야 하고, 그 **실행**이 1회다. CI 에서 무제한으로 두드리는 길은
"코드를 안 고치고 재실행"(위 표)과 "재실행 버튼을 누르지 않는다"(§6)로 막힌다.
...
- **답이 없으면 재촉하지 않는다.** 그 이슈는 대기 상태로 두고 다른 이슈로 가도 된다 —
  루프는 이슈마다 따로다 (§1). 기다림은 회차를 쓰지 않는다 (§2)
...
- CI 까지 갔다면 **사람이 고른 갈래 댓글**(§6)도 같이 읽는다 — 왜 ② 였는지가 거기에만 있다
...
| ⑥-b | 다른 AI (별도 컨텍스트) | 참인 것이 **계약이 시킨 그것인가** — 자문일 뿐 판정이 아니다 |
| ⑦ | 사람 | 승인 = 머지 |
...
**PR 에 커밋을 더할 때도 같다** — 열 때만이 아니라 **CI 로 가는 모든 코드가 로컬 verify 를 먼저 거친다**.
CI 실패는 회차를 쓰지 않으므로(§2), 이 줄이 없으면 고친 코드를 밀어 CI 만 다시 돌리는 길이
**한 회차도 쓰지 않고** 열린다.

### CI

- **CI 실패는 회차를 더 쓰지 않는다** (§2). 같은 코드를 다른 환경에서 재판정한 것이라 새 회차가 아니고,
  고쳐서 로컬 verify 를 돌리면 그 **실행**이 1회다
...
**CI 통과 1건당 한 번**이다 — 같은 초록불에 대고 다시 부르지 않는다.
...
|---|---|---|
| 4줄 `✓` · 범위 `✓` | **① 머지** | `n/N 사용` |
| `⚠` 하나 이상 · 계약은 그대로 | **② 다시 구현** | 고쳐서 verify 를 돌릴 때 **n+1** (§2) |
| 종료 조건이 배경(1번) 의도와 다름 | **③ 사람 판단** | 차감 없음 |
| `?` 만 있고 `⚠` 는 없음 | **보류** — 근거를 확인하고 고른다 | 고르기 전까지 변동 없음 |

권고는 **아래 세 갈래 중 하나를 가리킬 뿐** 새 갈래를 만들지 않는다. 고르는 것은 사람이다.
**실행** — 같은 세션이 `gh run watch` 로 CI 통과를 확인한 뒤(§6 CI), **별도 컨텍스트**
(서브에이전트 또는 새 세션)로 리뷰를 돌려 네 줄을 이슈 댓글에 단다.
**CI 잡을 새로 등록하지 않는다** — 리뷰는 판정이 아니라 CI 에 들어갈 자격이 없고,
잡이 되면 초록불 · 빨간불이 되어 자문이 게이트로 변한다.
갈라야 하는 것은 세션이 아니라 **컨텍스트**다 — 구현하며 세운 합리화가 넘어오지 않으면 된다.
...
| 갈래 | 언제 | 어디로 | 회차 |
|---|---|---|---|
| **① 머지** | CI 통과 + 사람 승인 | ⑦ 머지 = **루프 종료** → ⑧ **사람이** 이슈를 닫는다 | `n/N 사용` 기록 |
| **② 다시 구현** | CI 실패(로컬 **재현됨**) · 또는 CI 통과 + 리뷰 `⚠` — 「요구사항대로 안 됐다」 | ③ 구현 | 고쳐서 verify 를 돌릴 때 **n+1** (§2) — 계약이 그대로면 AI 의 몫 (§3-2) |
...
**고른 갈래는 이슈 댓글 한 줄로 남긴다.** 판정(④ · ⑥)과 자문(⑥-b)은 기록이 남지만
**선택은 사람 머릿속에만 남기 때문**이다. 권고와 다르게 골랐어도 그대로 적는다 —
자문을 언제 무시했는지가 쌓여야 리뷰의 값어치를 사후에 잴 수 있다.
...
| **확정** | `contract-draft` 라벨이 남아 있으면 아직 계약이 아니다 — 사람이 떼고 `계약 확정` 댓글 (§1) |

...
**흐름:** 요청자가 [요청 폼](../../.github/ISSUE_TEMPLATE/maintenance.yml)에 **1~4 (배경 · 변경할 내용 · 종료 조건 · 테스트 코드 기능명)** 를 적어 Issue 를 연다 → 에이전트가 **5·6 과 모호한 항목**을 댓글로 정리한다 → 사람이 확정하면 시작한다.
폼이 계약서의 앞 4개다. 기계 검증은 `npm run verify` 고정이므로, 요청자는 **그 안의 어느 테스트가 판정하는지**만 4번에 적는다.

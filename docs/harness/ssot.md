...
**흐름:** 요청자가 [요청 템플릿](../../.github/ISSUE_TEMPLATE/maintenance.md)에 **1~4 (배경 · 변경할 내용 · 종료 조건 · 테스트 코드 기능명)** 를 적어 Issue 를 연다 → 에이전트가 **5·6 과 모호한 항목**을 댓글로 정리한다 → 사람이 확정하면 시작한다.
템플릿은 마크다운 한 칸이다 — 밖에서 써 온 계약서를 통째로 붙여넣어도 된다. 기계 검증은 `npm run verify` 고정이므로, 요청자는 **그 안의 어느 테스트가 판정하는지**만 4번에 적는다.
...
| 4 | **기계 검증** | 무엇이 완료를 판정하나 |
| 5 | 건드리면 안 되는 것 | 경계는 어디인가 |
| 6 | 구현 루프 한도 | 몇 번까지 재시도하나 |

...
**흐름:** 요청자가 [요청 폼](../../.github/ISSUE_TEMPLATE/maintenance.yml)에 **작업 계약 6개 항목**을 적어 Issue 를 연다 → 비거나 모호한 항목은 에이전트가 댓글로 초안을 정리한다 → 사람이 확정하면 시작한다.
폼이 계약서다. 3·4·5 는 기본값이 채워져 있어 요청자는 고치기만 하면 된다.

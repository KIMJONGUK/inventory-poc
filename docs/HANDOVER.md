...
| ⑬ | CI **통과 뒤**에 **LLM review 를 자문**으로 (⑥-b) | 종료 조건을 오독하면 그 해석대로 테스트를 써서 실패 없이 통과한다 — 회차로도 CI 로도 못 잡는다. 앞(PR 전)에 두면 AI 가 사실을 정하게 되고 리뷰 왕복이 무한해지므로, **사실이 난 뒤 · 사람이 고르기 전**에 세운다. 회차를 쓰지 않고, 권고는 기존 세 갈래를 가리킬 뿐이다 |
